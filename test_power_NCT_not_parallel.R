...
#do the NCT
  ...
  NCT_pval_nwinv <- test_NCT$nwinv.pval

  return(NCT_pval_nwinv)
}
...
result_NCT_name <- paste0(gsub("\\.RData$", "", basename(result1)),"_", gsub("\\.RData$", "", basename(result2)), "_NCT.RData")

save(result, time_parallel, file = paste0("/dss/dsshome1/03/ga27hec2/NetworkSimulationAndComparison/results_power_NCT/", result_NCT_name))
